 ...
 * fetch: the `useBrowsePoints` hook (list / detail / map dots), the city
 * machine's `ensureBrowseReady` gate, and the nav prefetch. Keyed by slug +
 * immutable snapshot id; the provider defaults give
...
export function browsePointsQueryOptions(slug: string, snapshotId: string) {
  return {
    ...
    queryFn: ({ signal }: { signal?: AbortSignal }) =>
      fetchJson<BrowseCollection>(cityAssetUrl(slug, snapshotId, "points"), {
        signal,
      }),
  };
}
...
/**
 * The city machine's `ensureBrowseReady` gate, closured over the app
